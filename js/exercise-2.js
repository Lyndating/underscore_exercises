...
console.log(_.groupBy(words, function(w){ return w[0]}));
// - Check to see if all the words have more than 3 characters
console.log(_.reject(words,function(word){ return word.length <= 3}));
// - Check if some words are over twenty characters long
console.log(_.filter(words, function(word){ return word.length > 20;}));
// - Get an array of all of the uids in ` people `
console.log( _.map(people, function(p) {return _.pick(p, "uid")}));
// - Find the person with the highest uid
console.log(_.max(people, function(p) {return p.uid}));
// - Return an object that says how many even numbers and how many odd numbers there are in ` numbers `
console.log(_.countBy(numbers, function(num){ return num % 2 ==0 ? "even" : "odd";}));
